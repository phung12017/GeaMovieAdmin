...
exports._showMovieEdit = async (req, res) => {
    ...
    const movie = await Movie.find({ _id: req.params._id }, function (err, movie) {
    ...
    const cateMovie = await Category_Movie.find({movie_id:req.params._id}, function (err, cateMovie) {
        if (err) {
            return [err]
        } else {
            return cateMovie
...
exports._addMoviePostBody = async (req, res) => {
    ...
    var { name, director, country, language, years, duration, description, cover_img, trailer } = req.body
